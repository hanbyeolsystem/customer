...
- **Q&A 분류 `synology`(시놀로지 사용법·설정, 30문항 `syn-*`, 2026-09-10)**: 네이버 지식iN 실제 질문 80건에서 기존 nas 77문항과 겹치지 않는 것만 뽑음. 설치·연결 / 외부 접속·공유 / 권한·계정 / 백업·안전 / 용량·하드 / 활용·고장. DSM 메뉴 이름은 실제 이름만, 불확실한 수치는 "대개". 분류를 더 만들 때 손댈 곳 = qna.ts(type·qnaCats), qna/cat/[cat] INTRO, qna-images POOL, qna/[slug] GUIDE_CAT·CASE_CAT, lib/photos.ts pool.
...
- "대구 나스"는 검색에서 화장품(NARS)·상호와 겹치는 다의어라 Service.alternateName·llms.txt '이 사이트가 답하는 검색어'·/nas/ 요약문에 "나스(NAS, 네트워크 저장장치)"를 명시해 둔다. 지우지 말 것.
...
- **없는 사례·없는 수치를 만들어 넣지 말 것.** 구축사례는 원문 후기 링크가 있어야 하고,
  실적 수치(200곳/50건/300대/19년)는 `site.ts` 기준이며 임의로 부풀리지 않는다. 이게 무너지면
...
## 헤더 메뉴·첫 화면 (2026-09-15 첫 화면 점검 뒤 정리, 되돌리지 말 것)
- 헤더 메뉴는 `src/data/site.ts` 의 `nav` 한 곳. **4묶음**(서비스▾ · 구축사례 · Q&A·가이드▾ · 고객지원▾) + 오른쪽 전화 + [견적 문의]. 예전 14개 한 줄(13px)로 되돌리지 말 것. PC 첫 화면에서 누를 것이 21개(헤더 18)였고 사장님이 "복잡하다"고 했다.
- 새 페이지를 헤더에 바로 올리지 말고 묶음 안(items)에 넣는다. 묶음 항목의 desc 는 한 줄 설명.
