...
export default async function NasAreaPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const answer = isProvince
    ? `경상북도에서 나스(NAS, 네트워크 저장장치) 설치·구축·수리 업체를 찾으신다면 대구 달서구의 한별시스템(${site.phone.main})이 경북 전역으로 직접 출장합니다. 시놀로지(Synology) 공식 대리점으로 정품 NAS 판매와 RAID 설계, 3-2-1 백업, 랜섬웨어 대비, 직원 교육까지 현장에서 처리하며, 경북은 당일 출장을 원칙으로 합니다. 견적은 전화가 아니라 현장을 보고 무료로 내고, 예천·안동 등 경북 현장 사례와 구미·경산·칠곡·포항 설치 글이 사이트에 있습니다.`
  ...
  const facts = [
    ...
    { label: "방문", value: isProvince ? "경북 전역 당일 원칙" : a.policy === "same-day" ? "당일 출장 원칙" : "일정 잡아 방문" },
      ...
      <section className="py-10 lg:py-14 bg-[var(--bg)]">
        <div className="max-w-3xl mx-auto px-4 lg:px-6">
          ...
          <ol className="space-y-3">
            {[
              ["전화 한 통", `${site.phone.main}로 인원과 대략의 자료량, 지금 자료가 어디에 있는지 말씀해 주시면 방문 일정을 잡습니다. ${a.policy === "same-day" ? "경북은 당일 출장 원칙입니다." : "경남은 일정을 잡아 방문합니다."}`],
