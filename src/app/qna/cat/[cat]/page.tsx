...
const INTRO: Record<string, { title: string; desc: string; answer: string; facts: { label: string; value: string }[] }> = {
  ...
  nas: {
    ...
    answer: "NAS는 회사 안에 두는 전용 저장 서버로 클라우드와 달리 한 번 구축하면 회사 소유가 됩니다. 한별시스템은 대구·경북에서 50건 이상 구축했고 DS925+가 가장 많이 나가는 모델입니다. DS925+ 본체 1,066,000원에 8TB 2개와 출장 설치·설정교육 400,000원을 더하면 2,684,000원(VAT 별도)이며, 임대는 월 100,000원부터입니다. RAID는 백업이 아니므로 스냅샷과 외부 사본을 반드시 얹고, 랜섬웨어 대비 4가지(스냅샷·외부 사본·2단계 인증·인터넷 노출 차단)는 설정만으로 됩니다.",
    facts: [{ label: "NAS 구축", value: "50건 이상" }, { label: "DS925+ 구성", value: "2,684,000원" }, { label: "임대", value: "월 10만원부터" }, { label: "설치비", value: "400,000원" }],
  },
  printer: {
