...
const INTRO: Record<string, { title: string; desc: string; answer: string; facts: { label: string; value: string }[] }> = {
  ...
  nas: {
    ...
    answer: "NAS는 회사 안에 두는 전용 저장 서버로 클라우드와 달리 한 번 구축하면 회사 소유가 됩니다. 한별시스템은 대구·경북에서 100건 이상 구축했고 DS925+가 가장 많이 나가는 모델입니다. DS925+ 본체 1,402,000원에 8TB 2개와 출장 설치·설정교육 400,000원을 더하면 3,250,000원(VAT 별도)이며, 임대는 월 100,000원부터입니다. RAID는 백업이 아니므로 스냅샷과 외부 사본을 반드시 얹고, 랜섬웨어 대비 4가지(스냅샷·외부 사본·2단계 인증·인터넷 노출 차단)는 설정만으로 됩니다.",
    facts: [{ label: "NAS 구축", value: "100건 이상" }, { label: "DS925+ 구성", value: "3,250,000원" }, { label: "임대", value: "월 10만원부터" }, { label: "설치비", value: "400,000원" }],
  },
  synology: {
    title: "시놀로지 NAS 사용법·설정 Q&A",
    desc: "네트워크 드라이브 연결, 외부 접속, 권한·계정, Hyper Backup, 스냅샷, 용량 정리, 하드 이전, 경고음까지 실제로 많이 묻는 30가지.",
    answer: "시놀로지 NAS를 쓰면서 가장 많이 묻는 질문은 접속이 안 될 때(대부분 공유기·IP 문제), 밖에서 접속하는 방법(QuickConnect·DDNS·VPN), 직원 실수 삭제 막기(권한·휴지통·스냅샷), 백업이 실제로 되고 있는지 확인하는 법, 용량이 찼을 때 정리 순서, 본체 고장 때 하드를 새 본체로 옮기는 방법입니다. 대부분 설정으로 해결되고 초기화가 필요한 경우는 드뭅니다. 한별시스템(대구 달서구, 시놀로지 공식 대리점)은 설치 때 이 설정들을 기본으로 잡고 유지관리 거래처는 원격으로 먼저 확인합니다. 확실하지 않은 버튼은 누르지 말고 053-588-7119로 전화 주십시오.",
    facts: [{ label: "문항", value: "30개" }, { label: "근거", value: "지식iN 실제 질문" }, { label: "초기화", value: "대부분 불필요" }, { label: "전화", value: "053-588-7119" }],
  },
  printer: {
  ...
  pc: {
    ...
    answer: "사무용 PC는 하는 일에 맞는 사양이면 충분하고 무조건 고사양은 돈만 더 듭니다. 데스크탑과 모니터 세트는 월 40,000원부터(데스크탑 35,000원 + 모니터 5,000원, VAT 별도) 임대할 수 있고 AS가 포함됩니다. PC가 느린 원인이 PC가 아니라 네트워크인 경우가 많아 한별시스템은 원격으로 먼저 원인을 좁힌 뒤 방문합니다. 컴퓨터·복합기·NAS·네트워크를 한 회사가 관리해 관리 고객사 500곳 이상을 운영하고 있습니다.",
    facts: [{ label: "PC+모니터", value: "월 4만원부터" }, { label: "관리 고객사", value: "500곳 이상" }, { label: "대응", value: "원격 우선" }, { label: "대구·경북", value: "당일 출장" }],
  },
  network: {
    title: "네트워크·인터넷 Q&A",
  ...
  service: {
    ...
    answer: "한별시스템은 견적을 전화가 아니라 현장 방문으로 내고 방문 견적은 무료입니다. 대구·경북은 당일 출장 가며 예천·안동·포항·군위·칠곡·경산과 경남 창원 사무실까지 시공했습니다. 설치가 끝이 아니라 설치한 현장을 주기적으로 방문해 디스크 상태와 백업 동작을 확인하고, 필요 없는 곳에는 필요 없다고 말씀드립니다. 2008년 대구 성서공단에서 시작해 18년째이며 사업자등록번호 514-22-73057, 대표번호 053-588-7119입니다.",
    facts: [{ label: "방문 견적", value: "무료" }, { label: "대구·경북", value: "당일 출장 가능" }, { label: "업력", value: "2008년부터" }, { label: "전화", value: "053-588-7119" }],
  },
};
