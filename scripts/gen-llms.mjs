...
const SERVICES = `1. **시놀로지 NAS 판매·구축·데이터 백업** - 시놀로지 공식 대리점으로 정품 NAS 본체와 하드디스크를 판매·납품(본체 ${BODY_LOW}부터, 사무실 표준 구성은 하드와 출장 설치까지 ${BUY_FROM}부터, VAT 별도, 판매 페이지 ${SITE}/nas/buy/). 시놀로지 NAS 설치, RAID 설계, 3-2-1 백업 구성, 랜섬웨어 대비, VPN 원격접속, DSM 보안 설정과 분기 점검까지. 구축 실적 50개사 이상(네트워크 공사·백업 구축과 같은 현장 기준).
